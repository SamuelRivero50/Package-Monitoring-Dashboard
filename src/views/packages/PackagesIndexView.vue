...
const visiblePackages = computed<PackageInterface[]>(() => {
  ...
  if (!userId) return [];
  return allPackages.value.filter((pkg) => pkg.user.id === userId);
});

const selectorStatuses = computed<PackageStatus[]>(() => {
  const set = new Set<PackageStatus>();
  for (const pkg of visiblePackages.value) set.add(pkg.status);
...
const filteredPackages = computed<PackageInterface[]>(() =>
  visiblePackages.value.filter((pkg) => {
    const matchStatus =
      !selectedStatus.value || pkg.status === selectedStatus.value;
    const matchWarehouse =
      !selectedWarehouseId.value || pkg.warehouse.id === selectedWarehouseId.value;
...
function renderChart(): void {
  ...
  for (const pkg of visiblePackages.value) {
